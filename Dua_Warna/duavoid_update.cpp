...
//g++ -o xe lost.cpp $(pkg-config opencv4 --cflags --libs) -lrealsense2 
...
void on_mouse_click(int event, int x, int y, int flags, void* ptr) {
    ...
    if (event == cv::EVENT_LBUTTONDOWN) {
       ...
       if (first_click) {
            ...
            rectangle(snapshot_with_rect, roi, cv::Scalar(0, 0, 255), 2); 
            ...
            Mat colorArray;
            //COLOR_ROWS = 80; // Example value, adjust as needed
            //COLOR_COLS = 250; // Example value, adjust as needed
            ...
            imshow("Color 1", colorArray);

        }
        else {
            roi2.width = x - roi2.x;
            roi2.height = y - roi2.y;
            first_click = true;

            // Gambar kotak batasan di sekitar area yang dipilih
            Mat* snapshot = (Mat*)ptr;
            Mat snapshot_with_rect = snapshot->clone();
            rectangle(snapshot_with_rect, roi2, cv::Scalar(0, 255, 0), 2); 
            imshow("Snapshot", snapshot_with_rect);

            // Ambil region of interest (ROI) dari gambar snapshot berdasarkan kotak batasan
            Mat region_of_interest = (*snapshot)(roi2);
            ...
            Scalar mean_hsv = cv::mean(hsv_roi);
            hue2 = mean_hsv[0];
            saturation2 = mean_hsv[1];
            cout << "Rataan Nilai Hue 2: " << hue2 << endl;
            cout << "Rataan Nilai Saturation 2: " << saturation2 << endl;
            ...
            int g2 = mean_rgb[1];
            int b2 = mean_rgb[0];
            cout << "Rataan Nilai Rgb 2: [" << r2 << ", " << g2 << ", " << b2 << "]" << endl;
            string rgbText2 = "[" + to_string(r2) + ", " + to_string(g2)
                + ", " + to_string(b2) + "]";

            float luminance2 = 1 - (0.299*r2 + 0.587*g2 + 0.114*b2) / 255;
            Scalar textColor2;
            if (luminance2 < 0.5) {
            textColor2 = cv::Scalar(0,0,0);
            } else {
            textColor2 = cv::Scalar(255,255,255);
            }

            Mat colorArray2;
    
            colorArray2 = cv::Mat(COLOR_ROWS, COLOR_COLS, CV_8UC3, cv::Scalar(b2,g2,r2));
            putText(colorArray2, rgbText2, cv::Point2d(20, COLOR_ROWS - 20),
            FONT_HERSHEY_SIMPLEX, 0.8, textColor2);
            imshow("Color 2", colorArray2);
        }
...
void padi(){
 while (true) {
        ...
        imshow("Segmented Image2_Padi", segmented_image2);//ini
        imshow("Gabun_Padi", combined_mask2);
...
void bola(){
    while (true) {
        ...
        Scalar lower(nilai_warna - konstanta1, smin, vmin);
        Scalar upper(nilai_warna + konstanta2, smax, vmax);
