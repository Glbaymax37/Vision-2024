...
void line_follower(){
    ...
    while(true){
        ...
        Scalar whitelow(whmin,wsmin,wvmin);
        Scalar whitehigh(whmax,wsmax,wvmax);
        

        inRange(imgray,whitelow,whitehigh,whitemask);
        threshold(imgray, thresh, 100, 255, cv::THRESH_BINARY_INV);

        findContours(whitemask, contours, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE);
...
int main(int argc, char** argv) {
    ...
    namedWindow("Trackbars", (640, 200));
        ...
        createTrackbar("hmin", "Result", &whmin, 255);
        createTrackbar("smin", "Result", &wsmin, 255);
        createTrackbar("vmin", "Result", &wvmin, 255);
        createTrackbar("hmax", "Result", &whmax, 255);
        createTrackbar("smax", "Result", &wsmax, 255);
        createTrackbar("vmax", "Result", &wvmax, 255);


    

  bool isPadiRunning = false;
  bool isbolaRunning = false;
