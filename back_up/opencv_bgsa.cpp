...
int lowhue1 = 166, smin1 = 147,vmin1 = 0;
int upperhue1 = 180, smax1 = 255,vmax1 = 255;

int lowhuepadi = 164, sminpadi = 105, vminpadi = 0;
int upperhuepadi = 178 ,smaxpadi = 255, vmaxpadi = 255;


int lowhue2 = 121, smin2 = 73,vmin2 = 61;
int upperhue2 = 135, smax2 = 255,vmax2 = 255;
...
public:
    RadiusSubscriber() : Node("bola_subscriber") {
        subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
            "Command", 10, std::bind(&RadiusSubscriber::handleMessage, this, std::placeholders::_1));
    }
...
int contours_count_total = 0;
 

Rect detectionRegion(100, 340, 360, 350);
...
void bola(){   
    ...
    // while (rclcpp::ok) {
        ...
        for (size_t i = 0; i < contoursobject1.size(); ++i) {
            area = contourArea(contoursobject1[i]);
            if (area > maxArea) {
                maxArea = area;
        ...
        if (maxAreaIdx != -1 && maxArea > 100) {
            ...
            int contours_count1 = 0;

           

            // Draw the detection region on the frame
            rectangle(combinedFrame, detectionRegion, Scalar(0, 255, 0), 2);

            
            if (detectionRegion.contains(objectCenter)) {
            putText(combinedFrame, "*", objectCenter, FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 255, 255), 2);
            contours_count1++;
            // std::cout << contours_count1 << endl;
            ...
            bola_merah = contours_count1;
            
            message.linear.x = (320 - barang1.x)*45/320 ;  // Kecepatan linier pada sumbu x
            message.linear.y = barang1.y;
            message.linear.z = bola_merah;
        }
       else{
            message.linear.x = 0;  // Kecepatan linier pada sumbu x
            message.linear.y = 0;
            message.linear.z = 0;
       }
        ...
        for (size_t i = 0; i < contoursobject2.size(); ++i) {
             area2 = contourArea(contoursobject2[i]);
            if (area2 > maxArea2) {
                maxArea2 = area2;
        ...
        if (maxAreaIdx2 != -1 && maxArea2 > 100) {
           ...
           barang2 = objectCenter2;
            ...
            if (detectionRegion.contains(objectCenter2)) {
            putText(combinedFrame, "*", objectCenter2, FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 255, 255), 2);
            contours_count2++;
             //std::cout << contours_count2 << endl;


            }
           bola_ungu = contours_count2; 
           
        //    geometry_msgs::msg::Twist message;

            // message.linear.x = (320 - barang1.x)*45/320;  // Kecepatan linier pada sumbu x
            // message.linear.y = barang1.y;
            // message.linear.y = area;
            message.linear.z = 0;
            message.angular.x = (320 - barang2.x)*45/320;  // Kecepatan linier pada sumbu x
            message.angular.y = barang2.y;  // Kecepatan linier pada sumbu y
            // message.angular.y = area2;  
            message.angular.z = bola_ungu;
            // publisher->publish(message);
            cout << "area 2 = " << area << "mm" << endl;
            cout << "area 2 = " << area2 << "mm" << endl;
        }
        else{
            // geometry_msgs::msg::Twist message;
            message.angular.x = 0;  // Kecepatan linier pada sumbu x
            message.angular.y = 0;  // Kecepatan linier pada sumbu y
            message.angular.z = 0;
            // publisher->publish(message);
        }

        contours_count_total = bola_merah + bola_ungu;
...
//  rclcpp::WallRate loop_rate(10);
while (true) {
    //  cout << "Received message value in loop: " << posangularz << std::endl;
    keyVal = cv::waitKey(1) & 0xFF;
