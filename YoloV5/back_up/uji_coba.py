...
class CameraPublisher(Node):
    ...
    def camera_callback(self):
        ...
        if silo_coordinate:
           ...
           silo_msg.linear.y = float(bbox_area)   # Menentukan y coordinate di tengah bounding box
    ...
    def determine_silo_coordinate(self, detected_silos, detected_red_balls):
        ...
        if detected_red_balls:
            for silo in detected_silos:
                silo_tuple = tuple(silo)  # Ubah koordinat silo menjadi tuple
                red_balls_count[silo_tuple] = 0  # Inisialisasi jumlah red balls untuk setiap silo
                for red_ball in detected_red_balls:
                    ...
                    if distance < min_distance:
                        ...
                        closest_silo = silo
                    # Jika red ball ini dekat dengan silo, tambahkan ke count
                    if distance < threshold_distance:
                        red_balls_count[silo_tuple] += 1

            # Temukan silo terdekat yang belum memiliki tiga red ball
            for silo in detected_silos:
                silo_tuple = tuple(silo)  # Ubah koordinat silo menjadi tuple
                if red_balls_count[silo_tuple] < 2:  # Ganti 2 dengan angka yang sesuai
                    closest_silo = silo
                    break
