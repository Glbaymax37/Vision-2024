...
void Silo() {
   rclcpp::NodeOptions options;
    ...
    VideoCapture cap(0); // 0 untuk webcam utama, bisa diganti dengan path video
    ...
    //  while (rclcpp::ok()) {
        ...
        int y2 = y1 + 240;    
        // Ubah warna ke HSV
        visual(cv::Rect(x1, y1, 320, 240)).copyTo(displayFrame(cv::Rect(x1, y1, 320, 240)));

        Mat gray;

        Mat thres = cv::Mat::zeros(visual.size(), visual.type());
        ...
        cv::Mat frame3 = cv::Mat::zeros(visual.size(), visual.type());
        ...
        if (maxAreaIdx != -1 && maxArea > 50) {
            ...
            rectangle(visual, boundingBox, Scalar(255, 0, 0), 2);
            ...
            circle(visual, circleCenter, circleRadius, Scalar(0, 0, 255), 2);

            // Point Object1
            Point objectCenter(boundingBox.x + boundingBox.width / 2, boundingBox.y + boundingBox.height / 2);
            Point frameCenter(visual.cols / 2, visual.rows);

            //Rectangle background object 1
            Size textSize = getTextSize("Object1", FONT_HERSHEY_SIMPLEX, 0.5, 2, nullptr);
            rectangle(visual, Point(boundingBox.x, boundingBox.y - textSize.height - 10), Point(boundingBox.x + textSize.width, boundingBox.y), Scalar(255, 0, 0), FILLED);

            //garis dan keterangan
            putText(visual, "Object1", Point(boundingBox.x, boundingBox.y - 5), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255), 2);
            line(visual, frameCenter, objectCenter, Scalar(0, 255, 0), 2);
            circle(visual, Point(objectCenter), 5, Scalar(100, 255, 100), FILLED);

            // Koordinat Object 1
            string coordinateText1 = "(" + to_string(objectCenter.x) + ", " + to_string(objectCenter.y) + ")";
            putText(visual, coordinateText1, Point(objectCenter.x - 20, objectCenter.y - 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(250, 50, 255), 2);

            //Rectangle background Depth
            rectangle(visual, Point(500, 430), Point(640, 470), Scalar(200, 0, 0), FILLED);
            //Rectangle Keterangan
            rectangle(visual, Point(550, 410), Point(640, 430), Scalar(0, 0, 0), FILLED);

            putText(visual, "SILO", Point(560, 425), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255), 2);
...
int main(int argc, char** argv) {
 ...
 rclcpp::WallRate loop_rate(10);
    ...
    std::thread siloThread(Silo);
    std::thread padiThread(bola);
